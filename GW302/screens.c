...
// C Source File:    screens.c
// Description:      Graphic Wizard Screens
// -----------------------------------------------------------
...
unsigned char reverttime, lastoutput, nextoutput, MWarn, OKWarnBox;
...
{
	static unsigned char roll, CropFeedSel;
	...
	if( key_status ) // New key pressed or previous key repeated
	{
		...
		if( lkey == SW_CD )
		{
			window = screen100; // Goto Machine Menu
			Screen100Page = 0;
		}
	...
	if( (SecondScreen && (pRun.pr.LubeCounter > 0)) ||      // ( Second Screen AND if Lube Count Alarm is NOT present ) OR
	...
	if( SecondScreen && (pRun.pr.LubeCounter > 0) ) // Second screen & Lube Count Alarm is NOT present
	{
	...
	else  // Second Screen
	{
		...
		if( !(looptime % 5) )
		{
			TempFloatSV = ((float)AT91C_BASE_ADC0->ADC_CDR7 * ATODIPVOUTSCALE) + ATODIPVOUTDROP;
		}
	...
	if( pRun.pr.LubeCounter > 0 ) // Lube Count Alarm NOT present
	{
		...
		display_f1_text( tstring, TSMAX, 3, 24+2, FIXED );
		display_f1_text( "m", TSMAX, 3+(6*4)+2, 24+2, FIXED );
	}
...
void screen100( unsigned int lkey )
{
	...
	{		
		...
		else if( lkey == SW_IR ) // inner-right button (softkey ENT)
		{
			...
			{
			case 0: /* (1,1) */
				window = screen110; // Bale Setup Menu
				break;

			case 1: /* (2,1) */
				window = screen111; // Machine Setup Menu
				break;

			case 2: /* (1,2) */ 
				window = screen112; // Bale Count Menu
				break;

			case 3: /* (2,2) */
				window = screen113; // Operator Setup Menu
				break;
			}
		}
...
void screen110( unsigned int lkey )
{
	...
	if( startup != 1 )
	{
		...
		write_bitmap( adm, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_X, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_Y, OR );
		write_bitmap( adm, ICON_CORE_DENSITY,   ICON_CORE_DENSITY_X,  ICON_CORE_DENSITY_Y,  OR );
		write_bitmap( adm, ICON_BALE_DENSITY,   ICON_BALE_DENSITY_X,  ICON_BALE_DENSITY_Y,  OR );
	...
	if( selection != old_selection )
	{
		...
		{
		case 0: /* (1,1) */
			write_bitmap( adm, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_X, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_Y, AND );
		...
		case 2: /* (3,1) */ 
			...
			break;
		case 3: /* (1,2) */ 
			write_bitmap( adm, ICON_CORE_DENSITY, ICON_CORE_DENSITY_X, ICON_CORE_DENSITY_Y, AND );
			break;
		case 4: /* (2,2) */
			write_bitmap( adm, ICON_BALE_DENSITY, ICON_BALE_DENSITY_X, ICON_BALE_DENSITY_Y, AND );
			draw_line( adm, 159, 0, 159, 127, OR ); /* restoring right-hand frame border */
			break;
		case 5: /* (1,3) */ 
		...
		{
		case 0: /* (1,1) */
			write_bitmap( adm, ICON_BALE_PROFILE_SELECTED,  ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_X, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_Y, OR );
			break;
		case 1: /* (2,1) */ 
			write_bitmap( adm, ICON_CORE_DIAMETER_SELECTED, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_X, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_Y, OR );
			break;
		case 2: /* (3,1) */ 
			write_bitmap( adm, ICON_BALE_DIAMETER_SELECTED, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_X, ICON_BALE_PROFILE_CORE_DIAMETER_BALE_DIAMETER_Y, OR );
			break;
		case 3: /* (1,2) */ 
			write_bitmap( adm, ICON_CORE_DENSITY_SELECTED, ICON_CORE_DENSITY_X, ICON_CORE_DENSITY_Y, OR );
			break;
		case 4: /* (2,2) */
			write_bitmap( adm, ICON_BALE_DENSITY_SELECTED, ICON_BALE_DENSITY_X, ICON_BALE_DENSITY_Y, OR );
			break;
		case 5: /* (1,3) */ 
	...
	draw_box( adm, ICON_BALE_PROFILE_SETTING_X, ICON_BALE_PROFILE_SETTING_Y, ICON_BALE_PROFILE_SETTING_X2, ICON_BALE_PROFILE_SETTING_Y2, CLEAR );
	switch( pFac.pf.GW302_BaleProfile )
	{
	case BP_PRESET_A:
		write_bitmap( adm, ICON_BALE_PROFILE_A,  ICON_BALE_PROFILE_SETTING_X, ICON_BALE_PROFILE_SETTING_Y, OR );
		break;
	case BP_PRESET_B:
		write_bitmap( adm, ICON_BALE_PROFILE_B,  ICON_BALE_PROFILE_SETTING_X, ICON_BALE_PROFILE_SETTING_Y, OR );
		break;
	case BP_PRESET_C:
		write_bitmap( adm, ICON_BALE_PROFILE_C,  ICON_BALE_PROFILE_SETTING_X, ICON_BALE_PROFILE_SETTING_Y, OR );
		break;
	case BP_PRESET_D:
		write_bitmap( adm, ICON_BALE_PROFILE_D,  ICON_BALE_PROFILE_SETTING_X, ICON_BALE_PROFILE_SETTING_Y, OR );
		break;
	default: /* BP_PRESET_E */
		write_bitmap( adm, ICON_BALE_PROFILE_E,  ICON_BALE_PROFILE_SETTING_X, ICON_BALE_PROFILE_SETTING_Y, OR );
		break;
	} /* end switch( pFac.pf.GW302_BaleProfile ) */

	sprintf( tstring, "%3u", pFac.pf.GW302_CoreDiameter[pFac.pf.GW302_BaleProfile] );
	display_f1_text( tstring, TSMAX, ICON_CORE_DIAMETER_TEXT_X, ICON_CORE_DIAMETER_TEXT_Y, FIXED );

	sprintf( tstring, "%3u", pFac.pf.GW302_BaleDiameter[pFac.pf.GW302_BaleProfile] );
	display_f1_text( tstring, TSMAX, ICON_BALE_DIAMETER_TEXT_X, ICON_BALE_DIAMETER_TEXT_Y, FIXED );

	sprintf( tstring, "%3u", Bar[pFac.pf.GW302_CoreDensity[pFac.pf.GW302_BaleProfile] - 1] );
	display_f1_text( tstring, TSMAX, ICON_CORE_DENSITY_TEXT_X, ICON_CORE_DENSITY_TEXT_Y, FIXED );

	sprintf( tstring, "%3u", Bar[pFac.pf.GW302_BaleDensity[pFac.pf.GW302_BaleProfile] - 1] );
	display_f1_text( tstring, TSMAX, ICON_BALE_DENSITY_TEXT_X, ICON_BALE_DENSITY_TEXT_Y, FIXED );
	...
	if( half_second )
	{
		...
		case 0: /* (1,1) */
			...
			break;
		case 1: /* (2,1) */ 
			display_f1_text( "   ", TSMAX, ICON_CORE_DIAMETER_TEXT_X, ICON_CORE_DIAMETER_TEXT_Y, FIXED );
			break;
		case 2: /* (3,1) */ 
			display_f1_text( "   ", TSMAX, ICON_BALE_DIAMETER_TEXT_X, ICON_BALE_DIAMETER_TEXT_Y, FIXED );
			if( TryingToSetBaleDiaBelowCore )
			{
				display_f1_text( "   ", TSMAX, ICON_CORE_DIAMETER_TEXT_X, ICON_CORE_DIAMETER_TEXT_Y, FIXED ); // Flash off
			}
			break;
		case 3: /* (1,2) */ 
			display_f1_text( "   ", TSMAX, ICON_CORE_DENSITY_TEXT_X, ICON_CORE_DENSITY_TEXT_Y, FIXED );
			break;
		case 4: /* (2,2) */
			display_f1_text( "   ", TSMAX, ICON_BALE_DENSITY_TEXT_X, ICON_BALE_DENSITY_TEXT_Y, FIXED );
			break;
		case 5: /* (1,3) */ 
...
#define ICON_TWINE_ZERO_Y      	 	MACHINE_SETUP_MENU_ICONS_ORIGIN_Y + ICON_TWINE_ZERO_OFFSET_Y
...
#define PAGE_Y2    ICON_PRE_WARNING_BEEP_Y + 19  - 1  /* ICON_PRE_WARNING_BEEP bitmap Y dim - 1    */   
...
void screen111( unsigned int lkey )
{
	...
	{		
		...
		else if( lkey == SW_OL ) // outer-left button (softkey -)
		{
			if( page == 0 )
			{
				switch( selection )
				{
				case 0: /* Page 1 - Grid position (1,1) - Diameter Correction*/
					// -
					...
					break;
				case 1: /* Page 1 - Grid position (1,2) - Net Delay */
					// -
					...
					break;
				case 2: /* Page 1 - Grid position (1.3) - Net Metres */
					// RESET 
					window = screen125;	
					break;
			...
			else /* page == 1 */
			{
				...
				case 1: /* Page 2 - Grid position (1,2) - Bale Shape Indicator Zero */
					// RESET 
					window = screen124; // Fill Pot Zero Reset Screen (AKA Bale Shape Indicator Zero)
					break;
		...
		else if( lkey == SW_CD ) // navigation-up button (softkey V)
		{
			...
			{
				if( pFac.pf.GW302_TwineOption == TO_ON ) /* Only displayed if 'Twine Option' set to On.*/
				{
					if( selection < 2 )
					{
						selection++;
	...
	if( page != old_page )
	{
		...
		if( page == 0 )
		{
			write_bitmap( adm, ICON_DIAMETER_CORRECTION, ICON_DIAMETER_CORRECTION_X,  ICON_DIAMETER_CORRECTION_Y,  OR );
			write_bitmap( adm, ICON_NET_DELAY,           ICON_NET_DELAY_X,            ICON_NET_DELAY_Y,            OR );
			write_bitmap( adm, ICON_NET_METRES,          ICON_NET_METRES_X,           ICON_NET_METRES_Y,           OR );
		...
		else /* page == 1 */
		{
			...
			if( pFac.pf.GW302_TwineOption == TO_ON ) /* Only displayed if 'Twine Option' set to On.*/
			{/*write_bitmap( adm, ICON_FILL_POT_ZERO,       ICON_TWINE_ZERO_X,        	  ICON_TWINE_ZERO_Y,           OR );*/
				switch( pFac.pf.GW302_NetTwineOption )
				{
				case TO_TWINE:
					write_bitmap( adm, ICON_NET_TWINE_WITH_TWINE_HIGHLIGHTED, ICON_TWINE_ZERO_X, ICON_TWINE_ZERO_Y, OR );
	...
	if( selection != old_selection )
	{
		if( page == old_page )
		{
			...
			if( page == 0 )
			{
				...
				case 0: /* Page 1 - Grid position (1,1) */
					...
					break;
				case 1: /* Page 1 - Grid position (1,2) */ 
					write_bitmap( adm, ICON_NET_DELAY, ICON_NET_DELAY_OFFSET_X, ICON_NET_DELAY_OFFSET_Y, AND );
					break;
				case 2: /* Page 1 - Grid position (1.3) */ 
					write_bitmap( adm, ICON_NET_METRES, ICON_NET_METRES_OFFSET_X, ICON_NET_METRES_OFFSET_Y, AND );
					break;
				case 3: /* Page 1 - Grid position (1,4) */ 
			...
			else /* page == 1 */
			{
				...
				case 2: /* Page 2 - Grid position (1,3) -- not reached if 'Twine Option' set to Off */
					...
					draw_box( adm, ICON_TWINE_ZERO_X - 12, ICON_TWINE_ZERO_Y + 4, ICON_TWINE_ZERO_X - 2, ICON_TWINE_ZERO_Y + 20, CLEAR );
					break;
				}
			}
		}
		...
		if( page == 0 )
		{
			...
			{
			case 0: /* Page 1 - Grid position (1,1) */
				write_bitmap( adm, ICON_DIAMETER_CORRECTION_SELECTED, ICON_DIAMETER_CORRECTION_OFFSET_X, ICON_DIAMETER_CORRECTION_OFFSET_Y, OR );
				break;
			case 1: /* Page 1 - Grid position (1,2) */ 
				write_bitmap( adm, ICON_NET_DELAY_SELECTED, ICON_NET_DELAY_OFFSET_X, ICON_NET_DELAY_OFFSET_Y, OR );
				break;
			case 2: /* Page 1 - Grid position (1.3) */ 
				write_bitmap( adm, ICON_NET_METRES_SELECTED, ICON_NET_METRES_OFFSET_X, ICON_NET_METRES_OFFSET_Y, OR );
				break;
			case 3: /* Page 1 - Grid position (1,4) */ 
	...
	if( page == 0 )
	{
		...
		display_number( (float)pFac.pf.GW302_DiameterCorrection,
				...
				1.0, DP0, ICON_DIAMETER_CORRECTION_TEXT_X, ICON_DIAMETER_CORRECTION_TEXT_Y,
		...
		display_number( (float)pFac.pf.GW302_NetDelay,
				...
				1.0, DP1, ICON_NET_DELAY_TEXT_X, ICON_NET_DELAY_TEXT_Y,
				DW3, LEFT, &RDSfnt1, NULL, PNULL );

		// Net Metres
		display_number( (float)((unsigned int)pFac.pf.GW302_TotalNetMetres),
				(float)PF_GW302_TotalNetMetres[PF_MIN], (float)PF_GW302_TotalNetMetres[PF_MAX],
				1.0, DP0, ICON_NET_METRES_TEXT_X, ICON_NET_METRES_TEXT_Y,
	...
	if( half_second )
	{
		...
		if( page == 0 )
		{
			...
			{
			case 0: /* Page 1 - Grid position (1,1) */
				display_f1_text( "   ", TSMAX, ICON_DIAMETER_CORRECTION_TEXT_X -(3*6), ICON_DIAMETER_CORRECTION_TEXT_Y, FIXED );
				break;
			case 1: /* Page 1 - Grid position (1,2) */ 
				display_f1_text( "   ", TSMAX, ICON_NET_DELAY_TEXT_X, ICON_NET_DELAY_TEXT_Y, FIXED );
				break;
			case 2: /* Page 1 - Grid position (1.3) */ 
				display_f1_text( "     ", TSMAX, ICON_NET_METRES_TEXT_X -(5*6), ICON_NET_METRES_TEXT_Y, FIXED );
				break;
			case 3: /* Page 1 - Grid position (1,4) */ 
...
/*			case 2:  Page 2 - Grid position (1,3)
				if( pFac.pf.GW302_TwineOption == TO_ON )  Only displayed if 'Twine Option' set to On; nothing displayed here if 'Twine Option' set to Off
					display_f1_text( "   ", TSMAX, ICON_TWINE_TEXT_X, ICON_TWINE_TEXT_Y, FIXED );
				break;*/
			}
		}
	} /* end if( half_second ) */
	...
	{		
		...
		else if( lkey == SW_OL ) // outer-left button (softkey RESET)
		{
			if( selection < 10 ) /* Bale count, A - J */
			{
				...
				window = screen121;  /* Bale Count Reset Screen */
			}
			else if( selection == 10 ) /* Lube count */
			{
				window = screen120;  /* Lube Count Reset Screen */
			}
		}
		else if( lkey == SW_CU ) // navigation-up button (softkey ^)
		{
	...
	{		
		...
		else if( lkey == SW_IR ) // inner-right button (softkey ENT)
		{
			...
			case 0: /* (1,1) - Brightness & Contrast Adjust Menu */
				window = screen131; /* Brightness & Contrast Adjust Menu */
				break;

			case 1: /* (1,2) - Time & Date Adjust Menu */
				window = screen132; /* Time & Date Adjust Menu */
				break;

			case 2: /* (1,3) - Technician Menu */
#ifndef DEVELOP
				window = screen140; /* Pin Entry Display for Technician Menu */
#else
				window = screen141; /* Technician Menu */
#endif
				break;
			case 3: /* (1,4) - Diagnostics (Screens Access) */
				window = screen150; /* Diagnostics - Digital Inputs */
				break;
			}
		}
		else if( lkey == SW_CU ) // navigation-up button (softkey ^)
...
void screen131( unsigned int lkey  )  
{
	...
	if( change == 1 )
	{
		...
		// wait until SPI PDC ready (don't overwrite command registers)
		...
		command[0] = 0x81;
		command[1] = DISPLAY_DEFAULT_GAIN | pRun.pr.Contrast[pRun.pr.DayNight];
		// send command
...
void screen132( unsigned int lkey )
{
	...
	if(	ChangeTime )
	{
		ChangeTime = 0;
#ifndef WINDOWS
		if( xSemaphoreTake( SetTime, (portTickType)(2 / portTICK_RATE_MS) ) == pdTRUE ) // 2ms wait
		{
			// suspend task, else it will set time if it runs while we are setting ClockRam	
			vTaskSuspend( TWItaskhandle );

			// only reset seconds if adjusting minutes
			ClockRam[0] = (selection == 1) ? 0x00 : btobcd( t_sec ); // seconds
			ClockRam[1] = btobcd( t_min ); // minutes
			ClockRam[2] = btobcd( t_hrs ); // hours
			ClockRam[3] = btobcd( t_day ); // day of week
			ClockRam[4] = btobcd( t_date ); // day of month
			ClockRam[5] = ((t_year > 2099) ? 0x00 : 0x80) | btobcd( t_month ); // century & month
			ClockRam[6]  = btobcd( (unsigned char)(t_year % 100) ); // year   
			ClockRam[7] = 0x00; // Oscillation Adjust
			ClockRam[8] = 0x00; // Weekly Alarm minutes
			ClockRam[9] = 0x00; // Weekly Alarm hours
			ClockRam[10] = 0x00; // Weekly Alarm day of week
			ClockRam[11] = 0x00; // Daily Alarm minutes
			ClockRam[12] = 0x00; // Daily Alarm hours
			ClockRam[13] = 0x00; // Undefined
			ClockRam[14] = 0x20; // Control register 1 
			ClockRam[15] = 0x20; // Control register 2

			// resume task - resuming higher priority task causes this task to yield to TWI
			// TWI needs high priority else it could get interrupted during transfer
			vTaskPrioritySet( TWItaskhandle, mainTWI_TASK_SET_PRIORITY );
				...
				if( dig[3] == c )
				{
					...
					if( dig[2] == c )
					{
						...
						if( dig[1] == c )
						{
							...
							if( dig[0] == c )
							{
								window = Pop->PinWindow; /* Pin Secured Window */
							}
						}
					}
				}
...
void screen141( unsigned int lkey )
{
	...
	if( !startup )
	{
		...
		display_f1_text( "  Diameter Setup", TSMAX, 24, 41, FIXED );
		display_f1_text( "  Density Setup",  TSMAX, 24, 51, FIXED );
		display_f1_text( "  Knives Setup",   TSMAX, 24, 61, FIXED );
		display_f1_text( "  Reset Defaults", TSMAX, 24, 71, FIXED );
		display_f1_text( " ",                TSMAX, 24, 81, FIXED );
	...
	if( !editing && (lkey & (SW_NEW | SW_REP)) )
	{
		...
		{
			if( selection < 5 ) // not last [visible] option selected
			{
				selection++;
			}
			else if( swtim > 30 )
		...
		else if( (lkey == SW_IR) || (lkey == SW_CR) )
		{
			...
			{
			case 0:
				window = screen142; // Technician Menu - Baler Full
				break;

			case 1:
				window = screen143; // Technician Menu - Netting Setup
				break;

			case 2:
				window = screen144; // Technician Menu - Diameter Setup
				break;

			case 3:
				window = screen145; // Technician Menu - Density Setup
				break;

			case 4:
				window = screen146; // Technician Menu - Knives Setup
				break;
			...
			case 6:
				// No selection here -- selection pointer skips this space
				break;

			case 7:
				window = screen148; // Technician Menu - Formation 2
				break;
			}
		}
		else
...
void screen142( unsigned int lkey )
{
	...
	if( update_disp )
	{
		...
		else /* page == 1 */
		{
			...
			display_f1_text( "Engage Diameter", TSMAX, 24, 41, FIXED );
			display_f1_text( "%", TSMAX, 145, 41, FIXED );

			display_f1_text( "Bale Indicator", TSMAX, 24, 51, FIXED );

			display_f1_text( "Roller Delay", TSMAX, 24, 61, FIXED );
			display_f1_text( "s", TSMAX, 145, 61, FIXED );
		}
		update_disp = 0;
	...
	{		
		...
		else /* page == 1 */
		{
			...
			if( pFac.pf.GW302_BaleIndicator == BI_STEER )
				display_f1_text( "Steer", TSMAX, 125, 51, FIXED );
			else /* pFac.pf.GW302_BaleIndicator == BI_FILL */
				display_f1_text( " Fill", TSMAX, 125, 51, FIXED );
...
void screen143( unsigned int lkey )
{
		...
		if( lkey == SW_OL ) // -
		{
			...
			else /* page == 1 */
			{
				...
				case 2:
					...
					pFac.pf.GW302_NetTwineOption = TO_NET; /* 'Net/Twine' option reset to Net on turning 'Twine Option' On/Off */
	...
	if( update_disp )
	{
		...
		else /* page == 1 */
		{
			...
			display_f1_text( "Twine Option", TSMAX, 24, 41, FIXED );
		}
		update_disp = 0;
	...
	if( update_values )
	{
		...
		{	
			...
			if( pFac.pf.GW302_TwineOption == TO_OFF )
				display_f1_text( "OFF", TSMAX, 128, 41, FIXED );
			else /* pFac.pf.GW302_TwineOption == TO_ON */
					display_f1_text( "ON", TSMAX, 133, 41, FIXED );
				}
		update_values = 0;
...
void screen150 ( unsigned int lkey )
{
	...
	if( lkey & (SW_NEW | SW_REP) )
	{
		...
		if( lkey == SW_CR )
		{
			window = screen152; // Ananlog inputs
		}
		else if( (lkey == SW_OR) || (lkey == SW_CL) )
		{
			SavecalDelayWithinMenus = 1;
	...
	c = (AT91C_BASE_PIOB->PIO_PDSR & IP07) ? 1 : 0;
	write_bitmap( adm, ip_status[c], 31, 36, OR );

	c = (AT91C_BASE_PIOB->PIO_PDSR & IP08) ? 1 : 0;
	write_bitmap( adm, ip_status[c], 75, 36, OR );

	c = (AT91C_BASE_PIOB->PIO_PDSR & IP09) ? 1 : 0;
	write_bitmap( adm, ip_status[c], 117, 36, OR );

	c = (AT91C_BASE_PIOB->PIO_PDSR & IP10) ? 1 : 0;
	write_bitmap( adm, ip_status[c], 31, 58, OR );

	c = (AT91C_BASE_PIOB->PIO_PDSR & IP11) ? 1 : 0;
	write_bitmap( adm, ip_status[c], 75, 58, OR );

	c = (AT91C_BASE_PIOB->PIO_PDSR & IP12) ? 1 : 0;
...
void screen152 ( unsigned int lkey )
{
	...
	if( !startup )
	{
		...
		// set as voltage inputs
		AT91C_BASE_PIOB->PIO_SODR = IE_IP01 | IE_IP02 | IE_IP03 | IE_IP04;
		leaf = 1;
	}
	...
	// AnIP1: Bale Diameter
	sprintf( tstring, "%.2fV", BaleDiaVolts );
	display_f1_text( tstring, TSMAX, 46, 47, FIXED );

	if( BaleDiaVolts >= (POTDIV * 1.70) )
	{
		c = 0;
	}
	else if( BaleDiaVolts <= (POTDIV * 1.60) )
	{
		c = 1;
	}
	...
	sprintf( tstring, "%.2fV", DensityPresVolts );
	display_f1_text( tstring, TSMAX, 120, 47, FIXED );

	if( DensityPresVolts >= (POTDIV * 1.70) )
	{
		c = 0;
	}
	else if( DensityPresVolts <= (POTDIV * 1.60) )
	{
		c = 1;
	}
	...
	sprintf( tstring, "%.2fV", BaleShapeVolts );
	display_f1_text( tstring, TSMAX, 46, 81, FIXED );

	if( BaleShapeVolts >= (POTDIV * 1.70) )
	{
		c = 0;
	}
	else if( BaleShapeVolts <= (POTDIV * 1.60) )
	{
		c = 1;
	}
	...
	sprintf( tstring, "%.2fV", KnifePresVolts );
	display_f1_text( tstring, TSMAX, 120, 81, FIXED );

	if( KnifePresVolts >= (POTDIV * 1.70) )
	{
		c = 0;
	}
	else if( KnifePresVolts <= (POTDIV * 1.60) )
	{
		c = 1;
	}
...
void screen151 ( unsigned int lkey )
{
...
#ifndef WINDOWS	  
	AT91C_BASE_PIOA->PIO_SODR = OUT1;// clear all outputs
	AT91C_BASE_PIOA->PIO_SODR = OUT2;
	AT91C_BASE_PIOA->PIO_SODR = OUT3;
	AT91C_BASE_PIOA->PIO_SODR = OUT4;
	AT91C_BASE_PIOA->PIO_SODR = OUT5;
	AT91C_BASE_PIOA->PIO_SODR = OUT6;
	AT91C_BASE_PIOA->PIO_SODR = OUT7;
	AT91C_BASE_PIOA->PIO_SODR = OUT8;
#else
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT1;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT2;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT3;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT4;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT5;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT6;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT7;
	AT91C_BASE_PIOA->PIO_ODSR &= ~OUT8;
#endif

	if( (key & SW_IL) || (key & SW_OL) )
	{
		if( swtim < 20 )
			swtim++;
	...
	display_number( tempf, 0.0, 0.0, 1.0, DP1, 147, 4, DW4, RIGHT, &RDSfnt1, NULL, PNULL );

#ifndef WINDOWS 
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT1) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT1) ? 1 : 0;
#endif
	write_bitmap( adm, (selection==0)? op_status[c]: BUTTON03, 32, 28, OR );

#ifndef WINDOWS 
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT2) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT2) ? 1 : 0;
#endif	
	write_bitmap( adm, (selection==1)? op_status[c]: BUTTON03, 77, 28, OR );

#ifndef WINDOWS 
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT3) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT3) ? 1 : 0;
#endif	
	write_bitmap( adm, (selection==2)? op_status[c]: BUTTON03, 119, 28, OR );

#ifndef WINDOWS 
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT4) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT4) ? 1 : 0;
#endif	
	write_bitmap( adm, (selection==3)? op_status[c]: BUTTON03, 32, 48, OR );

#ifndef WINDOWS 
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT5) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT5) ? 1 : 0;
#endif	
	write_bitmap( adm, (selection==4)? op_status[c]: BUTTON03, 77, 48, OR );

#ifndef WINDOWS 
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT6) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT6) ? 1 : 0;
#endif	
	write_bitmap( adm, (selection==5)? op_status[c]: BUTTON03, 119, 48, OR );

#ifndef WINDOWS
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT7) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT7) ? 1 : 0;
#endif
	write_bitmap( adm, (selection==6)? op_status[c]: BUTTON03, 32, 68, OR );

#ifndef WINDOWS
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT8) ? 0 : 1;
#else
	c = (AT91C_BASE_PIOA->PIO_ODSR & OUT8) ? 1 : 0;
#endif
